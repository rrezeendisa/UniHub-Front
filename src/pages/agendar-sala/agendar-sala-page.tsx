...
import { useReservationRoomForm } from '../../hooks/useReservationRoomForm'
...
export function AgendarSalaPage() {
  ...
  const { computedRooms, isLoading, refreshRooms, getAvailabilityMap } = useRoomAvailability()
  ...
  const { submit: submitReservation, isLoading: isReserving } = useRoomReservation(selectedRoom, {
  ...
  const {
    ...
    onSubmit: async (data) => {
      console.log('[DEBUG] onSubmit AgendarSalaPage iniciado')
      try {
        ...
        <main className="min-w-0 flex-1">
          <section className="overflow-hidden rounded-3xl border border-[var(--color-gray-light)] bg-[var(--color-surface)] p-5 shadow-sm sm:p-6 lg:p-6">
            ...
            <div className="mt-6 sm:mt-7">
              <RoomsGrid
                rooms={computedRooms.map(cr => cr.room)}
